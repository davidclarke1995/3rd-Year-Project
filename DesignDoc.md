...
### Added Functionality
Functionality added after the video was made included increased database sercuity by encrypting passwords with AES encryption which is inbuilt into mySql. The database interface also now checks if the username entered is already in the database and prompts the user to enter a new, unique username.
